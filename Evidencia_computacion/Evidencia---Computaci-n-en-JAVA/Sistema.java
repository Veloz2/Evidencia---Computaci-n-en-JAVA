...
public class Sistema {
    ...
    public void altaDoctor(){
        ...
        Archivo.writeLines("db/doctores.txt", id + "," + nombre);
        ...
        Archivo.writeLines("db/pacientes.txt", id + "," + nombre);
    ...
    public void crearCita(){
        ...
        System.out.println("Ingresa el siguiente formato: hh:mmAM/PM-dd/mm/aaaa");
        ...
        boolean pacienteExiste = existe("db/pacientes.txt", idPaciente);
        boolean doctorExiste = existe("db/doctores.txt", idDoctor);

            if(!pacienteExiste){
                System.out.println("El paciente con ID: "+ idPaciente + " no existe.");
                System.out.println("Desea agregar al paciente: (Y/N):");
            ...
            if(!doctorExiste){
                System.out.println("El doctor con ID " + idDoctor + " no existe");
                System.out.println("Desea agregar al paciente: (Y/N):");
                String select = sc.nextLine();
                select.toUpperCase();
                if(select.equalsIgnoreCase("Y")){
        ...
        if(pacienteExiste && doctorExiste){
            Archivo.writeLines("db/citas.txt",
        idCita + "," + fecha + "," + motivo + "," + idPaciente + "," + idDoctor);
        System.out.println("Cita guardada en el sistema\n");
        } else{
             System.out.println("No se pudo generar la cita porque faltan datos.\n");
        }
    }
    ...
    public void mostrarDatos(){
        ...
        for (String linea : Archivo.readLines("db/doctores.txt")){
            String[] d = linea.split(",");
            System.out.println("ID: "+ d[0] + " | Nombre: "+ d[1]);
            System.out.println("--------------------------");
        }
        System.out.println(" ----- PACIENTES -----");
        for (String linea : Archivo.readLines("db/pacientes.txt")){
            String[] p = linea.split(",");
            System.out.println("ID: "+ p[0] + " | Nombre: "+ p[1]);
            System.out.println("--------------------------");
        }
        System.out.println(" ----- CITAS -----");
        for(String linea : Archivo.readLines("db/citas.txt")){
